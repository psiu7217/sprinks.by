<?php
class ControllerExtensionWmModuleWmSeoPro extends Controller {
    ...
    $this->cache_data = $this->wmlicense->cache();
	...
	public function index() {
		...
		if ($this->config->get('config_wm_seo_url')) {
		...
		if (!isset($this->request->get['_route_'])) {
			...
			if (isset($this->request->get['product_id'])) {
				...
				if (!isset($this->request->get['path'])) {
					$path = $this->model_extension_wm_module_wm_seo_pro->getPathByProduct($this->request->get['product_id']);
	...
	public function rewrite($link) {
		if (!$this->config->get('config_wm_seo_url')) return $link;
		...
		switch ($route) {
			case 'product/product':
				if (isset($data['product_id'])) {
					$tmp = $data;
					$data = array();
					if ($this->config->get('config_wm_seo_url_include_path')) {
						$data['path'] = $this->model_extension_wm_module_wm_seo_pro->getPathByProduct($tmp['product_id']);
						if (!$data['path']) return $link;
					}
					$data['product_id'] = $tmp['product_id'];
					$seo_pro_utm = preg_replace('~\r?\n~', "\n", $this->config->get('config_wm_seo_url_utm'));
			...
			case 'product/category':
				if (isset($data['path'])) {
					...
					$data['path'] = $this->model_extension_wm_module_wm_seo_pro->getPathByCategory($category);
					if (!$data['path']) return $link;
				}
				break;
		...
		if ($this->config->get('config_wm_seo_url_https')) {
		...
		if (isset($postfix)) {
			$seo_url .= trim($this->config->get('config_wm_seo_url_postfix'));
		} else {
			$seo_url .= '/';
		}
